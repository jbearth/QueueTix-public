...
import { formatDate } from 'utils/convertDateAndTime';
...
const TABLE_HEAD: readonly TABLE_HEAD_PROPS[] = [
  { id: 'title', label: 'Title', alignRight: 'left' },
  { id: 'discountchild', label: 'Discount Child', alignRight: 'left' },
  { id: 'discountadult', label: 'Discount Adult', alignRight: 'left' },
  { id: 'startDate', label: 'Start Date', alignRight: 'left' },
  { id: 'endDate', label: 'End Date', alignRight: 'left' },
...
const DELETE_PROMOTION = gql`
  mutation DeletePromotion($idPromotion: String!) {
    DeletePromotion(id_promotion: $idPromotion) {
      success {
        message
      }
      error {
...
export default function PromotionListPage() {
  ...
  // Query
  const {
    loading: queryPromotionAllLoading,
    error: queryPromotionAllError,
    data: queryPromotionAllData,
    refetch
  } = useQuery(PROMOTIONALL);

  // Mutation
  const [deleteRowTable] = useMutation(DELETE_PROMOTION);

  if (queryPromotionAllLoading) {
    return (
  ...
  if (queryPromotionAllError) {
    return `Error! ${queryPromotionAllError.message}`
  };
  ...
  const handleSelectAllClick = (event: { target: { checked: any; }; }) => {
    if (event.target.checked) {
      const newSelecteds: any = queryPromotionAllData.GetPromotionAll.map((employee: any) => employee.firstname);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - queryPromotionAllData.GetPromotionAll.length) : 0;


  const filteredEmployee = applySortFilter(queryPromotionAllData.GetPromotionAll, getComparator(order, orderBy), filterSearch)

  const isNotFound = !filteredEmployee.length && !!filterSearch;

  const handleDeleteRow = async (e: React.SyntheticEvent) => {
    e.preventDefault();
    try {
      setAnchorElActions(null); // ปิด popover
      await deleteRowTable({ variables: { idPromotion: stateNavigate.selectedRowId! } });
  ...
  return (
    <>
      ...
      {/* Title Tab on Web */}
      <Helmet>
        <title> List Promotion | Management </title>
      </Helmet>
      ...
      <Container maxWidth={drawerOpen ? "lg" : "xl"}>
        <Button
          ...
          onClick={() => navigate("/dashboard/management/promotion/create")}
        >
          Add Promotion
        </Button>
        ...
        >
          ...
          {/* เครื่องมือจัดการ Table Promotion Filter */}
          <UserListToolbar
            theme={theme}
          ...
          <TableContainer component={Paper}>
            ...
            >
              ...
              <UserListHead
                ...
                rowCount={queryPromotionAllData.GetPromotionAll.length}
                numSelected={selected.length}
                onRequestSort={handleRequestSort}
                onSelectAllClick={handleSelectAllClick}
              />
              <TableBody sx={{ '& .Mui-selected': { bgcolor: theme.palette.primaryAlpha } }}>
                {filteredEmployee.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row: TablePromotionProps) => {
                  const { id, title, description, discountchild, discountadult, startDate, endDate, picture } = row;
          ...
          >
            ...
            <TablePagination
              ...
              component="div"
              count={queryPromotionAllData.GetPromotionAll.length}
      ...
      >
        ...
        <MenuItem
          onClick={() => {
            navigate(`/dashboard/management/promotion/edit/${stateNavigate.selectedRowId}`, {
              state: {
