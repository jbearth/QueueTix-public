...
const TABLE_HEAD: readonly TABLE_HEAD_PROPS[] = [
  ...
  { id: 'GENDER', label: 'GENDER', alignRight: 'left' },
  { id: 'types', label: 'TYPES', alignRight: 'left' },
  { id: 'rides', label: 'RIDES', alignRight: 'left' },
...
const DELETE_ACCOUNT = gql`
  mutation DeleteEmployee($idEmployee: String!) {
    deleteEmployee(id_employee: $idEmployee) {
      success {
        message
      }
      error {
...
export default function EmployeeListPage() {
  ...
  // Query
  const {
    loading: queryEmployeeAllLoading,
    error: queryEmployeeAllError,
    data: queryEmployeeAllData,
    refetch
  } = useQuery(EMPLOYEEALL);

  // Mutation
  const [deleteRowTable] = useMutation(DELETE_ACCOUNT);

  if (queryEmployeeAllLoading) {
    return (
  ...
  if (queryEmployeeAllError) {
    return `Error! ${queryEmployeeAllError.message}`
  };
  ...
  const handleSelectAllClick = (event: { target: { checked: any; }; }) => {
    if (event.target.checked) {
      const newSelecteds: any = queryEmployeeAllData.getEmployeeAll.map((employee: any) => employee.firstname);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - queryEmployeeAllData.getEmployeeAll.length) : 0;


  const filteredEmployee = applySortFilter(queryEmployeeAllData.getEmployeeAll, getComparator(order, orderBy), filterSearch)
  ...
  const handleDeleteRow = async (e: React.SyntheticEvent) => {
    ...
    try {
      ...
      await deleteRowTable({ variables: { idEmployee: stateNavigate.selectedRowId! } });
  ...
  return (
    <>
      ...
      {/* Title Tab on Web */}
      <Helmet>
        <title> List Employee | Management </title>
      </Helmet>
      ...
      <Container maxWidth={drawerOpen ? "lg" : "xl"}>
        <Button
          ...
          onClick={() => navigate("/dashboard/management/employee/create")}
        >
          Add Employee
        </Button>
        ...
        >
          ...
          <TableContainer component={Paper}>
            ...
            >
              ...
              <UserListHead
                ...
                rowCount={queryEmployeeAllData.getEmployeeAll.length}
                numSelected={selected.length}
                onRequestSort={handleRequestSort}
                onSelectAllClick={handleSelectAllClick}
              />
              <TableBody sx={{ '& .Mui-selected': { bgcolor: theme.palette.primaryAlpha } }}>
                {filteredEmployee.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row: TableUserProps) => {
                  const { id, role, email, firstname, lastname, types, gender, profilePicture, rides } = row;
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* คอลัมน์ Types */}
                      <TableCell align="left">
                        <Label color={types === "EntranceEmployee" ? "secondary" : "primary"}
                        >
                          {types}
                        </Label>
                      </TableCell>
          ...
          >
            ...
            <TablePagination
              ...
              component="div"
              count={queryEmployeeAllData.getEmployeeAll.length}
      ...
      >
        ...
        <MenuItem
          onClick={() => {
            navigate(`/dashboard/management/employee/edit/${stateNavigate.selectedRowId}`, {
              state: {
