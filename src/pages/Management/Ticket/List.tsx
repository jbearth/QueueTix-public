...
const TABLE_HEAD: readonly TABLE_HEAD_PROPS[] = [
  ...
  { id: 'description', label: 'Description', alignRight: 'left' },
  { id: 'types', label: 'Types', alignRight: 'left' },
  { id: 'priceofchild', label: 'Price of Child', alignRight: 'left' },
  { id: 'priceofadult', label: 'Price of Adult', alignRight: 'left' },
...
type TableTicketProps = {
  ...
  description: string;
  typesticket: {
    types: "DreamWorldVisa" | "SuperVisa" | "Entrance" | "IncludeRides";
    priceofchild: number;
    priceofadult: number;
  }
}
...
export default function TicketListPage() {
  ...
  const handleSelectAllClick = (event: { target: { checked: any; }; }) => {
    if (event.target.checked) {
      const newSelecteds: any = queryTicketAllData.GetTicketAll.map((ticket: any) => ticket.title);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - queryTicketAllData?.GetTicketAll?.length) : 0;


  const filteredRides = applySortFilter(queryTicketAllData.GetTicketAll, getComparator(order, orderBy), filterSearch)
  ...
  return (
    <>
      ...
      {/* Title Tab on Web */}
      <Helmet>
        <title> List Ticket | Management </title>
      </Helmet>
        ...
        >
          ...
          <TableContainer component={Paper}>
            ...
            >
              ...
              <UserListHead
                ...
                rowCount={queryTicketAllData.GetTicketAll?.length}
                numSelected={selected.length}
              ...
              <TableBody sx={{ '& .Mui-selected': { bgcolor: theme.palette.primaryAlpha } }}>
                {filteredRides.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row: TableTicketProps) => {
                  const { id, title, description, typesticket } = row;
                    ...
                    >
                      ...
                      <TableCell align="left" sx={{ paddingLeft: 1.8 }}>
                        <Stack direction={'row'} alignItems={'center'}>
                          <Iconify icon={"majesticons:ticket-text"} sx={{ marginRight: 1, color: theme.palette.secondary.main }} />
                          {typesticket.types}
                        </Stack>
                      </TableCell>
                      ...
                      <TableCell align="left">
                        ...
                        >
                          {typesticket.priceofchild}
                        </Label>
                      </TableCell>

                      {/* คอลัมน์ priceofadult */}
                      <TableCell align="left">
                        <Label color={"successCustom"}
                        >
                          {typesticket.priceofadult}
                        </Label>
                      </TableCell>
          ...
          >
            ...
            <TablePagination
              ...
              component="div"
              count={queryTicketAllData.GetTicketAll.length}
      ...
      >
        ...
        <MenuItem
          onClick={() => {
            navigate(`/dashboard/management/ticket/edit/${stateNavigate.selectedRowId}`, {
              state: {
